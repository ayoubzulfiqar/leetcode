# 🔥 Number of Good Paths 🔥 || 3 Approaches || Simple Fast and Easy || with Explanation
...
class B {
  ...
  int numberOfGoodPaths(List<int> vals, List<List<int>> edges) {
    ...
    // build Graph
    HashMap<int, HashSet<int>> graph = HashMap();

    for (List<int> edge in edges) {
      int u = edge[0];
      int v = edge[1];

      graph.putIfAbsent(u, () => HashSet<int>());
      graph.putIfAbsent(v, () => HashSet<int>());
